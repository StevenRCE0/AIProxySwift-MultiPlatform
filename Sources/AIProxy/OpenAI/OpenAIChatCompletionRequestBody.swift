...
nonisolated public struct OpenAIChatCompletionRequestBody: Encodable, Sendable {
    ...
    /// Output types the model can return, e.g. `["text", "audio"]`.
    /// Required when requesting audio output from audio-capable models like `gpt-audio-1.5`.
    public let modalities: [String]?

    /// Configuration for audio output. Required when `modalities` includes `"audio"`.
    /// Specifies the voice and format of the generated audio.
    public let audio: AudioOutputConfig?
    ...
    public init(
        ...
        audio: OpenAIChatCompletionRequestBody.AudioOutputConfig? = nil
    ) {
        self.model = model
...
extension OpenAIChatCompletionRequestBody {
    ...
    nonisolated public enum Message: Encodable, Sendable {
        ...
        public func encode(to encoder: any Encoder) throws {
            ...
            switch self {
            case .assistant(let content, let name, let refusal, let toolCalls, let audio):
...
extension OpenAIChatCompletionRequestBody.Message {
    nonisolated public enum ContentPart: Encodable, Sendable {
        ...
        /// Audio input content. `data` is base64-encoded audio, `format` is the audio format (e.g. "wav", "mp3").
        ...
        public func encode(to encoder: any Encoder) throws {
            ...
            case .inputAudio(let data, let format):
                ...
                var nestedContainer = container.nestedContainer(keyedBy: AudioKey.self, forKey: .inputAudio)
...
// MARK: - AudioOutputConfig
extension OpenAIChatCompletionRequestBody {
    /// Configuration for audio output when `modalities` includes `"audio"`.
    nonisolated public struct AudioOutputConfig: Encodable, Sendable {
        /// The voice to use for audio output (e.g. "alloy", "echo", "shimmer").
        public let voice: String

        /// The format of the audio output. Use `"pcm16"` for streaming, `"wav"` for non-streaming.
...
extension OpenAIChatCompletionRequestBody {
    ...
    /// Use the `id` from a prior response's `audio` object.
    nonisolated public struct AudioReference: Encodable, Sendable {
        public let id: String
